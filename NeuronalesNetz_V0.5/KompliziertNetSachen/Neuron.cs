...
namespace NeuronalesNetz_V0._5.KompliziertNetSachen
{
    public class Neuron
    {
        ...
        public double maxFactor = 5;
        public double minFactor = -5;
        ...
        public double OutputFactor
        {
            ...
            set
            {
                if (value < maxFactor && value > minFactor)
                    parameter[parameter.Length - 1] = value;                   
            }
        }
        public int SetMaxDelay
        {
            set
            {
                if (value > MaxDelay)
                {
                    lastOutputList.AddRange(new double[value - MaxDelay]);
                    lastDeltaList.AddRange(new double[value - MaxDelay]);
                    MaxDelay = value;
                }
                else if(value < MaxDelay)
                {
                    lastOutputList.RemoveRange(value - 1, MaxDelay - value);
                    lastDeltaList.RemoveRange(value - 1, MaxDelay - value);
                    MaxDelay = value;
                }
            }
            get
        ...
        public Neuron(double minFactor, double maxFactor, string bezeichnung0, params double[] parameter0)
        {
